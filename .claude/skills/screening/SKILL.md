...
### Step 6: Generate email opener

Use the `/email-subject-line` skill for the opener. Pass the candidate profile info to it.
The screening step saves the opener alongside the score — but the WRITING is handled by the email skill, not here.

## PHASE 3: SAVE RESULTS

### Output Format
```json
{"score": 7, "result": "qualified", "notes": "FIT: 8 years DevOps leadership at scale-ups, exact K8s + Terraform match. STRENGTH: Built platform team from 2 to 12. CONCERN: No GCP, all AWS.", "opener": "<generated by /email-subject-line skill>"}
```
